...
############################################
EXEC=./spmv_1d
CSV_MPI=1d_spmv_results_mpi.csv
CSV_HYB=1d_spmv_results_hybrid.csv
...
# Weak scaling: hybrid uses this OMP, MPI-only uses --no-omp
WEAK_OMP=4

# 1D supports both read modes
READ_MODES=(mpiio parallel root)
...
if [[ -n "$PBS_NODEFILE" && -f "$PBS_NODEFILE" ]]; then
    ...
    # Unique host list in allocation order
...
# Guard for weird interactive PBS nodefiles
...
extract_kv () {
  # extract_kv "$line" "KEY="
  # prints value after KEY= up to whitespace
  echo "$1" | awk -v key="$2" '
    {
      for(i=1;i<=NF;i++){
        if(index($i,key)==1){
...
# Goal: if it fits on 1 node, stay on 1 node
############################################
choose_nodes_and_ppn () {
    ...
    # If it fits in one node, force one-node run to avoid inter-node comm
    local used_nodes=1
...
############################################
# BUILD HOSTLIST (PBS only). For SLURM/none, leave empty.
############################################
build_hostlist () {
    local used_nodes=$1
...
run_test () {
    ...
    # MPI-only: threads are effectively 1, we pass --no-omp
    # Hybrid: threads >= 2, we pass --threads N
    ...
    # Decide used nodes + ppn (keep 1 node when possible)
    ...
    # Build hostlist (PBS only)
    HOSTLIST=$(build_hostlist "$USED_NODES")

    echo "Running: $mode | $scaling | read=$read_mode | $matrix | MPI=$ranks | OMP=$effective_threads | USED_NODES=$USED_NODES | PPN=$PPN"

    if [[ -n "$HOSTLIST" ]]; then
        output=$(mpiexec -hosts "$HOSTLIST" -np "$ranks" -ppn "$PPN" -bind-to core \
            "$EXEC" "$matrix" --read="$read_mode" $extra_args 2>&1)
    else
    ...
    status=$?
    if [[ $status -ne 0 ]]; then
        echo "MPI run failed (MODE=$mode read=$read_mode MPI=$ranks OMP=$effective_threads)"
        echo "$matrix,$scaling,$read_mode,$ranks,$effective_threads,ERROR,,,,,,,,," >> "$csv_out"
        return
    fi
    ...
    echo "$matrix,$scaling,$read_mode,$ranks,$effective_threads,OK,\
...
for matrix in "${STRONG_MATRICES[@]}"; do
  for read_mode in "${READ_MODES[@]}"; do
    for ranks in "${MPI_RANKS[@]}"; do
      # MPI-only
      run_test "$matrix" "strong" "$read_mode" "$ranks" 1 "mpi"
      # Hybrid (threads >= 2)
      for threads in "${OMP_THREADS_HYB[@]}"; do
        run_test "$matrix" "strong" "$read_mode" "$ranks" "$threads" "hybrid"
      done
    done
  done
done
...
for read_mode in "${READ_MODES[@]}"; do
  for ranks in "${MPI_RANKS[@]}"; do
    ...
    # MPI-only
    run_test "$matrix" "weak" "$read_mode" "$ranks" 1 "mpi"

    # Hybrid (fixed OMP)
    run_test "$matrix" "weak" "$read_mode" "$ranks" "$WEAK_OMP" "hybrid"
  done
done

echo "===== ALL TESTS COMPLETED (1D) ====="
